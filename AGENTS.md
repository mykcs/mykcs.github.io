# Repository Agent instructions

This repository is the public, redirect-only compatibility gateway for https://mykcs.github.io. It is not the complete personal-homepage source repository.

## Product wish authority

Before changing redirect behavior, route ownership, public copy, or the role of this repository, read docs/wish/LATEST.md.

- For route topology, hosting boundaries, or product-direction changes, also read docs/wish/DESIGN.md.
- Do not read docs/wish/ARCHIVE.md by default; use it only for historical intent.
- Shared Wish lifecycle rules live at https://github.com/mykcs/.codex/blob/main/website-governance/WISH_PROTOCOL.md.
- Current owner instructions, privacy/security boundaries, executable truth, and live hosting state outrank the Wish.

## Repository boundary

- Keep this repository public-safe and redirect-only.
- Do not copy the Astro application, private Git history, internal DEV tooling, credentials, unpublished material, or private assets here.
- Full homepage source authority: private repository mykcs/personal-homepage.
- Current canonical/indexable homepage: https://wangrui92.pages.dev.
- Redirect directly to the canonical serving host; do not route through another compatibility gateway.
- Known homepage routes may preserve their matching path.
